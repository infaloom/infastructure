...
```bash
export CNPG_BACKUP_S3_ENDPOINT_URL="{CHANGE_ME}"
```
:::warning
...
The above config will schedule a daily backup every midnight. Test the backups by running an ad-hoc backup (optional):
```bash
...
Useful utility to restart postgres cluster in case of configuration changes https://cloudnative-pg.io/documentation/1.15/cnpg-plugin/
```bash
